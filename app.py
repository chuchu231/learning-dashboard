...
# ---------------------- Gọi API ----------------------
BASE_URL = "https://1509ce019c41.ngrok-free.app"
    ...
    with col1:
        if not df_avg_time.empty:
            ...
            if (df_avg_time["AVG_QuizTime"] > 0).any():
                ...
                fig1.update_layout(margin=dict(l=30, r=30, t=40, b=20), height=300, plot_bgcolor="white")
    ...
    with col4:
        ...
        if not df_avg_question.empty:
            ...
            fig_rating = px.line(
                ...
                markers=True,
                color_discrete_sequence=["#68BA7F"]
            )
        
            fig_rating.update_traces(line=dict(width=2), marker=dict(size=9, line=dict(width=1, color="#2E6F40")))
